...
@Parser(name = "Data", wrappers = {Response.class})
public class DataParser<T> extends TypeParser<Response<T>> {

    //该构造方法是必须的
    protected DataParser() { super(); }
    //如果依赖了RxJava，该构造方法也是必须的
    public DataParser(Type type) { super(type); }
...
//        T t = (T) data; //获取data字段
        ...
        if (data.getMeta().getCode() != 0) {//这里假设code不等于0，代表数据不正确，抛出异常
